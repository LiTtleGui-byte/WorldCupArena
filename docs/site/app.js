...
function toggleTournamentPrediction(idx) {
  ...
  if (willShow && !panel.dataset.rendered) {
    // Lazy-render the (heavy, markdown-laden) bracket detail on first open instead of
    // eagerly building it into a hidden div for all 9 models on initial page render.
...
async function fetchDataForLanguage() {
  ...
  // Fast path: small "live" payload (everything except history). history is fetched
  // afterwards via _history_url, off the first-paint critical path. Falls through to the
  // full file when the split files aren't deployed (old build) — old behaviour preserved.
  try {
...
async function loadSiteData() {
  ...
  renderSiteData();
  // history ships in its own file (data.history.<lang>.json) to keep ~80% of the bytes off
  // the first-paint critical path; load it now and fill the history section when it arrives.
